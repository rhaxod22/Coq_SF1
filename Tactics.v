...
(* FILL IN HERE
    Do not need to write reflextivity.
    [] *)
...
Proof.
  ...
  injection H as H1 H2.
  assert (K : y :: l = z :: l).
    { rewrite H2. rewrite G. reflexivity. }
  injection K as K1.
  rewrite H1. rewrite K1. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros m H. destruct m.
    + discriminate H.
    + f_equal.
      apply IHn'. simpl in H. injection H as goal. rewrite <- plus_n_Sm in goal. rewrite <- plus_n_Sm in goal.
      injection goal as fin. apply fin.
Qed.      
(** [] *)
...
Proof.
  ...
  - intros n H. induction n.
    + discriminate H.
    + simpl. apply IHl. injection H as goal. apply goal.
Qed.
(** [] *)
...
Proof.
  ...
  - intros l1 l2 H. injection H as H1 H2. rewrite <-H1, <-H2. reflexivity.
  - destruct n as [n1 n2]. simpl. destruct (split l').
    intros l1 l2 H. injection H as H1 H2.
    rewrite <- H1, <- H2. simpl.
    assert (F: combine x y = l').
...
Lemma eqb_S_inj : forall (n m : nat),
  (n =? m) = (S n =? S m).
Proof.
  intros n. induction n.
  - simpl. reflexivity.
  - intros m. specialize IHn with (m := S m).
    apply S_inj. reflexivity.
Qed.
...
Proof.
  ...
  - destruct m.
    + reflexivity.
    + rewrite <- eqb_S_inj. rewrite <- eqb_S_inj. apply IHn.
  Qed.        
(** [] *)
...
Definition split_combine_statement : Prop :=
  ...
  match l1, l2 with
  | [], _ => combine l1 l2 = l -> split l = ([], [])
  | _, [] => combine l1 l2 = l -> split l = ([], [])
  | x :: tx, y :: ty => combine l1 l2 = l -> split l = (l1, l2)
  end. *)
  (* match (combine l1 l2) with
  | [] => combine l1 l2 = l -> split l = ([], [])
  | h :: t => combine l1 l2 = l -> split l = (l1, l2)
  end. *)

Theorem split_combine : split_combine_statement.
Proof.
  unfold split_combine_statement.
  intros X l1. induction l1 as [| n1 l1' IHl1'].
  - intros l2. destruct l2 as [|n2 l2'].
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
  ...
  test x = true.
Proof.
  intros X test x l. induction l as [|h t IH].
  - intros lf H. simpl in H. discriminate H.
  - intros lf. destruct lf as [|h' t'].
    + simpl. intros H. destruct (test h) eqn:testh.
      * injection H as H1. rewrite H1 in testh. apply testh.
      * apply IH in H. apply H.
    + simpl. intros H. destruct (test h) eqn:testh.
      * injection H as H1 H2. rewrite H1 in testh. apply testh.
      * apply IH in H. apply H.
Qed.
(** [] *)
...
Proof.
  intros X test l. unfold existsb. unfold existsb'.
  induction l as [|h t IH].
  - simpl. reflexivity.
  - destruct (test h) eqn:testh.
    + simpl. rewrite testh. simpl. reflexivity.
    + simpl. rewrite testh. simpl. apply IH.
Qed.
(** [] *)
