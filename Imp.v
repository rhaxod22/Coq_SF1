...
Proof.
  ...
  - intros H. 
    induction H;
    simpl; try (apply aeval_iff_aevalR in H); try (apply aeval_iff_aevalR in H0);
    subst; reflexivity.
...
Definition pup_to_n : com :=
  CSeq (CAsgn Y 0) (CWhile (BGt X 0) (CSeq (CAsgn Y (APlus Y X)) (CAsgn X (AMinus X 1))) ).
...
Proof.
  ...
  - intros H. induction c; try constructor.
    + simpl in H. apply andb_prop in H. destruct H as [H1 H2]. apply IHc1. apply H1.
    + simpl in H. apply andb_prop in H. destruct H as [H1 H2]. apply IHc2. apply H2.
    + simpl in H. apply andb_prop in H. destruct H as [H1 H2]. apply IHc1. apply H1.
    + simpl in H. apply andb_prop in H. destruct H as [H1 H2]. apply IHc2. apply H2.
    + simpl in H. discriminate H.
  - intros H. induction c; try reflexivity.
    + simpl. inversion H. subst. apply andb_true_intro. split.
      * apply IHc1. apply H2.
      * apply IHc2. apply H3.
    + simpl. inversion H. subst. apply andb_true_intro. split.
      * apply IHc1. apply H2.
...
Proof.
  ...
  - inversion H. subst.
    destruct (IHc1 H2 st) as [st1 G1].
    destruct (IHc2 H3 st1) as [st2 G2].
    exists st2. apply E_Seq with st1.
    + apply G1.
    + apply G2.
  - inversion H. subst.
    destruct (IHc1 H2 st) as [st1 G1].
    destruct (IHc2 H4 st) as [st2 G2].
    destruct (beval st b) eqn: B.
    + exists st1. apply E_IfTrue.
      * apply B.
      * apply G1.
    + exists st2. apply E_IfFalse.
      * apply B.
      * apply G2.
  - inversion H.
Qed.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | h :: t => match h with
              ...
              | SPlus =>  match stack with
                          ...
                          | n1 :: nil => s_execute st stack t
                          | n2 :: n1 :: nt => s_execute st ([n1+n2]++nt) t
                          end
...
Proof.
  ...
  - intros. induction a.
    + simpl. apply IHp1.
    + simpl. apply IHp1.
    + induction stack as [|n1 stack'].
      * simpl. apply IHp1.
      * induction stack' as [|n2 stack''].
        { apply IHp1. }
        { simpl. apply IHp1. }
    + induction stack as [|n1 stack'].
      * simpl. apply IHp1.
...
Proof.
  intros st e. induction e; try reflexivity.
  - simpl. rewrite execute_app. rewrite IHe1.
    rewrite execute_app.
...
Inductive ceval : com -> state -> result -> state -> Prop :=
  ...
  | E_SeqBreak : forall c1 c2 st st',
      st =[ c1 ]=> st' / SBreak ->
      st =[ c1; c2 ]=> st' / SBreak
  | E_SeqContinue : forall c1 c2 st st' st'',
      st =[ c1 ]=> st' / SContinue ->
      st' =[ c2 ]=> st'' / SContinue ->
  ...
  | E_IfFalse : forall st st' b c1 c2,
      beval st b = false ->
      st =[ c2 ]=> st' / SContinue ->
      st =[ if b then c1 else c2 end]=> st' / SContinue
  | E_IfTrue : forall st st' b c1 c2,
      beval st b = true ->
      st =[ c1 ]=> st' / SContinue ->
      st =[ if b then c1 else c2 end]=> st' / SContinue
...
Proof.
  ...
  - apply conj; reflexivity.
  - apply conj; reflexivity.
  - apply conj; try reflexivity.
    apply IHceval with st2 in H5. apply proj1 in H5. apply H5.
  - apply IHceval with st'0 in H2. destruct H2 as [G1 G2].
    discriminate G2.
  - apply IHceval1 with st2 in H6. destruct H6 as [G1 G2].
    discriminate G2.
  - apply conj; try reflexivity.
    apply IHceval1 with st'0 in H3 as [G1 G2]. subst.
    apply IHceval2 with st2 in H7 as [G _]. apply G.
  - apply conj; try reflexivity.
    apply IHceval with st2 in H8 as [G _]. apply G.
