...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  ...
  | h :: t => match (count h s2) with
              | O => false
              | S n => (included t (remove_one h s2))
              end
  end.
...
Theorem count_dist : forall n s1 : nat, forall s2 : bag,
  count n (s1 :: s2) = (count n [s1]) + (count n s2).
Proof. intros n s1 s2. induction s2.
  - simpl. induction (s1 =? n).
    + simpl. reflexivity.
    + simpl. reflexivity.
  - simpl. induction (s1 =? n).
    + induction (n0 =? n).
...
Proof. intros n s. induction s.
  - simpl. rewrite eqb_refl. reflexivity.
  - simpl. induction (n0 =? n).
    + simpl. assert (H: add n s = sum s [n]).
      { reflexivity. }
      rewrite <- H.
      rewrite IHs.
      simpl. reflexivity.
    + assert (H: add n s = sum s [n]).
      { reflexivity. }
      rewrite <- H.
      rewrite IHs.
      simpl. reflexivity.
...
Proof. intros s1 s2 n. induction s1.
  - simpl. reflexivity.
  - simpl. destruct (n0 =? n).
