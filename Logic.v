...
Theorem not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P HNP Q HP. unfold not in HNP. apply HNP in HP. destruct HP.
Qed.
(** [] *)
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q H NQ. unfold not. intros HP.
  unfold not in NQ. apply H in HP. apply NQ in HP. apply HP.
Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P. unfold not. intros [HP HNP]. apply HNP, HP.
Qed.
(** [] *)
...
Proof.
  ...
  destruct HQR as [H3 H4]. split.
  - intros HP. apply H1 in HP. apply H3 in HP. apply HP.
...
Proof.
  ...
  - intros H. destruct H as [HP|HQR].
    ...
    + destruct HQR as [HQ HR]. split.
      * right. apply HQ.
      * right. apply HR.
  - intros H. destruct H as [H1 H2]. destruct H1 as [HP|HQ].
    + left. apply HP.
...
Proof.
  intros X P HA. unfold not. intros [x HX].
  apply HX in HA. apply HA.
Qed.
(** [] *)
...
Lemma leb_pred : forall n m, (n <=? m = true) -> (pred n <=? pred m = true).
Proof.
...
Proof.
  intros n m.
  generalize dependent n. destruct m as [|m'] eqn:M.
  - simpl. intros n H. discriminate H.
  - simpl. intros n H. specialize leb_n_Sn' with m'. intros G.
    destruct n.
    + reflexivity.
    + simpl.  *)
...
Proof.
  ...
  - intros m H.
    assert (G: (S n' <=? m = true) -> (n' <=? m = true)).
    { specialize leb_n_Sn' with (n'). intros G. } *)
  intros n m. generalize dependent n. induction m as [|m' IHm'].
  - intros n H. destruct n.
    + exists 0. reflexivity.
    + simpl in H. discriminate H.
  - intros n H. specialize IHm' with (pred n). destruct n eqn:N.
    + simpl. exists (S m'). reflexivity.
    + apply leb_pred in H. simpl in H.
      apply IHm' in H. destruct H as [x Hx].
      exists (x). rewrite Hx.
      rewrite add_comm. rewrite plus_n_Sm. simpl.
      symmetry. rewrite add_comm. rewrite plus_n_Sm. reflexivity.
Qed.

Theorem plus_exists_leb : forall n m, (exists x, m = n+x) -> n <=? m = true.
Proof.
  intros n m H. destruct H as [x Hx]. destruct x.
  - rewrite add_comm in Hx. simpl in Hx. rewrite Hx. apply leb_refl.
  - rewrite <- plus_O_n with n. rewrite add_comm.
    rewrite Hx. apply plus_leb_compat_l. reflexivity.
Qed.
(** [] *)
  ...
  - induction l as [|x l' IHl'].
    ...
    + simpl. intros [H | H].
      * exists x. split.
        { apply H. }
        { left. reflexivity. }
      * apply IHl' in H. destruct H as [a [H1 H2]].
        exists a. split.
        { apply H1. }
        { right. apply H2. }
  - induction l as [|x l' IHl'].
    + intros [x [H1 H2]]. simpl in H2. simpl. apply H2.
    + simpl. intros [a [H1 H2]]. destruct H2 as [G | G].
      * left. rewrite <- G in H1. apply H1.
      * right. apply IHl'. exists a. apply conj. apply H1. apply G.
Qed.
(** [] *)
...
Proof.
  ...
  - intros l'' a. simpl. split.
    + intros [G | G].
      * left. left. apply G.
      * apply IH in G. destruct G as [G1 | G2].
        { left. right. apply G1. }
        { right. apply G2. }
    + intros [G | G].
      * destruct G as [G1 | G2].
        { left. apply G1. }
        { right. apply IH. left. apply G2. }
      * right. apply IH. right. apply G.
Qed.
(** [] *)
...
Proof.
  ...
  - induction l as [|x l' IHl'].
    ...
    + simpl. intros H. split.
      * specialize H with x. apply H. left. reflexivity.
      * apply IHl'. intros a.
        specialize H with a. intros Ha.
        apply H. right. apply Ha.
  - induction l as [|x l' IHl'].
    + simpl. intros H x contra. destruct contra.
    + simpl. intros [H1 H2].
      intros a. specialize IHl' with a. intros [G | G].
      * rewrite G in H1. apply H1.
      * apply IHl'. apply H2. apply G.
Qed.
(** [] *)
...
Proof.
  ...
  - rewrite even_S. destruct (even n') eqn:E.
    + simpl. destruct IHn' as [x Hx].
      exists x. rewrite Hx. reflexivity.
    + simpl. destruct IHn' as [x Hx].
      exists (x+1). rewrite Hx. 
      rewrite add_comm. rewrite plus_1_l with (x).
      rewrite double_incr. reflexivity.
Qed.    
(** [] *)
...
Proof.
  intros x y. split.
  - intros H. unfold not. intros G.
    rewrite G in H. rewrite eqb_refl in H. discriminate H.
  - unfold not. intros H. rewrite <- not_true_iff_false.
    unfold not. intros G. apply eqb_eq in G.
    apply H. apply G.
Qed.
(** [] *)
...
Lemma eqb_list_refl :
  forall A (eqb : A -> A -> bool) l,
    (forall a1 a2, eqb a1 a2 = true <-> a1 = a2) ->
    eqb_list eqb l l = true.
Proof.
  intros. induction l.
  - simpl. reflexivity.
  - simpl. destruct (eqb x x) eqn:E.
    + apply IHl.
    + specialize H with x x. destruct H as [H1 H2].
      rewrite E in H2. apply H2. reflexivity.
Qed.    
...
Proof.
  ...
  - generalize dependent l2. induction l1 as [|a l1' IHl'].
    + destruct l2.
      * simpl. reflexivity.
      * simpl. intros G. discriminate G.
    + destruct l2 as [|b l2'].
      * simpl. intros G. discriminate G.
      * simpl. destruct (eqb a b) eqn:E.
        { intros K. apply IHl' in K.
          specialize H with a b. apply H in E.
          rewrite E. rewrite K. reflexivity. }
        { intros K. discriminate K. }
  - intros G. induction l1 as [|a l' IHl'] eqn:EG.
    + simpl. rewrite <- G. reflexivity.
    + simpl. rewrite <- G. destruct (eqb a a) eqn:E.
      * apply eqb_list_refl. apply H.
      * specialize H with a a. destruct H as [H1 H2].
        rewrite E in H2. apply H2. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. induction l as [|x l' IHl'].
    + simpl. apply I.
    + simpl. simpl in H. destruct (test x) eqn: Tx.
      * split. reflexivity. apply IHl'. apply H.
      * discriminate H.
  - intros H. induction l as [|x l' IHl'].
    + simpl. reflexivity.
    + simpl. destruct (test x) eqn: Tx.
      * apply IHl'. simpl in H. destruct H as [_ H2]. apply H2.
      * simpl in H. destruct H as [H1 _]. rewrite Tx in H1. apply H1.
Qed.
...
Lemma tr_rev_correct_aux : forall (X : Type) (l1 l2 : list X),
  rev_append l1 l2 = rev_append l1 [] ++ l2.
Proof.
  intros X l1. induction l1 as [|a l1' IHl'].
  - simpl. reflexivity.
  - intros l2. simpl.
    rewrite IHl' with (a :: nil).
    rewrite IHl' with (a :: l2).
...
Theorem excluded_middle_irrefutable: forall (P : Prop),
  ~ ~ (P \/ ~ P).
Proof.
  unfold not. intros P H. apply H. right.
  intros G. apply H. left. apply G.
Qed.
(** [] *)
