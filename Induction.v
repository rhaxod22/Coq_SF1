...
Proof.
  ...
  rewrite mul_0_r.
  rewrite mult_n_1. rewrite add_0_r. reflexivity.

  rewrite IHn'.
  assert (H1: m + m * n' = m * S n').
  { rewrite temp. reflexivity. }
  rewrite H1.
  assert (H2: m * S (S n') = m + m * S n').
  { rewrite temp. reflexivity. }
...
Proof.
  ...
  simpl. rewrite <- IHn'. rewrite mul_comm_helper. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite mul_comm_helper. rewrite IHp'.
...
Proof.
  ...
  rewrite mul_comm_helper. rewrite mul_comm. rewrite mult_plus_distr_r.
  assert (H: m * p' * n = n * m* p').
  { rewrite mul_comm. rewrite IHp'. reflexivity. }
  rewrite H. rewrite mul_comm. rewrite <- mult_n_Sm.
  rewrite add_comm. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  rewrite IHn'. rewrite plus_1_l. reflexivity.
Qed.  
(** [] *)
...
Theorem n2b_twice : forall n, nat_to_bin (n + n) = double_bin (nat_to_bin n).
Proof.
...
Theorem incr_double : forall b, incr (double_bin b) = B1 b.
Proof.
  intros b. induction b.
  - simpl. reflexivity.
  - simpl. reflexivity.
