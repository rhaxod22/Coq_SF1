...
(* FILL IN HERE
    ...
    [] *)
Example Perm3_example2 : Perm3 [1;2;3] [1;2;3].
Proof.
...
Proof.
  intros n m. intros E1 E2. induction E2 as [|m' E2' IHE2'].
  - rewrite <- plus_n_O. apply E1.
  - apply ev_SS in IHE2'.
    assert (H : n + S (S m') = S (S (n + m'))).
...
Proof.
  ...
  - intros H. induction H.
    ...
    + assert (T: S (S n) = 2 + n).
      { reflexivity. }
      rewrite T. apply ev'_sum. apply ev'_2. apply IHev.
Qed.
(** [] *)
...
Proof.
  ...
  - apply IHE2.
    ...
    { simpl. reflexivity. }
    rewrite H in E1. apply evSS_ev in E1. apply E1.
Qed.
(** [] *)
...
Proof.
  ...
  assert (T: n + m + n + p = double n + m + p).
  { assert (temp: m + (n + p) = n + m + p).
    { rewrite <- add_assoc. rewrite add_comm.
      assert (temp1: p + m = m + p).
      { rewrite add_comm. reflexivity. }
      rewrite <-add_assoc. rewrite temp1.
      rewrite add_assoc. reflexivity. }
    rewrite <- add_assoc. rewrite <- add_assoc.
    rewrite temp. rewrite add_assoc. rewrite add_assoc.
    rewrite <- double_plus. reflexivity. }
  rewrite add_assoc in H. rewrite T in H.
...
Proof.
  intros m n o H1. generalize dependent o.
  induction H1 as [|mn H2 IH2].
  - intros o G. apply G.
  - intros o H3. apply IH2.
    induction H3 as [|o' H4 IH4].
    + apply le_S. apply le_n.
    + apply le_S. apply IH4.
Qed.
...
Proof.
  ...
  inversion HS.
  - apply le_n.
  - apply (le_trans n (S n) m (n_le_Sn n)) in H0.
    apply H0.
Qed.
...
Proof.
  ...
  - intros m. destruct m.
    ...
    + destruct (IHn' m) as [IH | IH].
      * left. apply n_le_m__Sn_le_Sm. apply IH.
      * right. apply n_le_m__Sn_le_Sm. apply IH.
Qed.  
...
Proof.
  ...
  - apply conj. apply le_plus_l. rewrite add_comm. apply le_plus_l.
  - destruct IHle as [H1 H2]. split.
    + apply le_S. apply H1.
    + apply le_S. apply H2.
Qed.

Lemma Sn_le_m : forall n m,
  S n <= m -> n <= m.
Proof.
...
Proof.
  ...
  - left. apply O_le_n.
  - destruct IHn'.
    + rewrite add_comm in H. rewrite <- plus_n_Sm in H. rewrite add_comm in H. 
      apply (le_trans (n' + m) (S (n' + m)) (p + q) (n_le_Sn (n' + m))) in H. apply H.
    + right. rewrite  add_comm, <- plus_n_Sm, add_comm in H. apply Sn_le_m in H.
  Admitted.
...
Proof.
  ...
  - simpl. apply MStarApp.
    ...
    + apply IHt. intros s G.
      apply H. simpl. right. apply G.
Qed.
(** [] *)
...
Proof.
  ...
  - intros H. induction re.
    ...
    + simpl in H.
      ...
      destruct (IHre1 H1) as [s1 Ha].
      destruct (IHre2 H2) as [s2 Hb].
      exists (s1++s2). apply MApp. apply Ha. apply Hb.
    + simpl in H.
      rewrite orb_true_iff in H. destruct H as [H | H].
      * destruct (IHre1 H) as [s1 HS].
        exists s1. apply MUnionL. apply HS.
      * destruct (IHre2 H) as [s2 HS].
        exists s2. apply MUnionR. apply HS.
    + exists []. apply MStar0.
Qed.
(** [] *)
...
Proof.
  ...
  - discriminate Heqre'.
  - discriminate Heqre'.
  - exists []. simpl. apply conj.
    reflexivity. intros s' contra. destruct contra.
  - destruct (IH2 Heqre') as [ss' [H2L H2R]].
    exists (s1 :: ss'). split.
    + simpl. rewrite <- H2L. reflexivity.
...
Proof.
  ...
  - split. reflexivity. intros G. apply HP.
  - split.
    + unfold not in HNP. intros HP. apply HNP in HP. destruct HP.
...
Proof.
  ...
  - simpl. destruct (eqbP n m).
    ...
    + simpl. intros. apply IHl'.
      * apply H0.
      * destruct H1 as [G | G].
        { exfalso. apply H. rewrite G. reflexivity. }
        { apply G. }
  (* - simpl. unfold not. intro contra. destruct contra.
  - simpl. destruct (eqbP n m) as [H | H].
    + rewrite H in Hcount. simpl in Hcount.
      rewrite eqb_refl in Hcount. discriminate Hcount.
    + intros G. destruct G as [G | G].
      * unfold not in H. apply H. rewrite G. reflexivity.
      * apply IHl' in G.
        { apply G. }
        { simpl in Hcount. destruct (n =? m) eqn:E.
          - apply eqb_eq in E. unfold not in H.
...
Proof.  apply nostutter_xy. apply eqb_neq. reflexivity.
        apply nostutter_xy. apply eqb_neq. reflexivity.
        apply nostutter_xy. apply eqb_neq. reflexivity.
        apply nostutter_xy. apply eqb_neq. reflexivity.
        apply nostutter_xy. apply eqb_neq. reflexivity.
...
Inductive merge {X:Type} : list X -> list X -> list X -> Prop :=
  | merge_nil : merge [] [] []
  | merge_left : forall (x : X) (l1 l2 l : list X),
      merge l1 l2 l -> merge (x :: l1) l2 (x :: l)
  | merge_right : forall (x : X) (l1 l2 l : list X),
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl.
    inversion H0. rewrite H2. f_equal.
    apply IHmerge. apply H3. apply H1.
  - simpl.
    inversion H1. rewrite H2.
    apply IHmerge. apply H0. apply H3.
Qed.
(** [] *)
...
*)
Lemma rev_l_r_x : forall {X: Type} (x : X) (l r: list X),
  l = rev r ++ [x] -> rev l = x :: r.
Proof.
  intros X x l r. generalize dependent l.
  induction r as [|x' r' IHr'].
  - simpl. intros l H. rewrite H. reflexivity.
  - simpl. intros l H.
    rewrite H. rewrite rev_app_distr. simpl. f_equal.
...
Inductive disjoint {X: Type} : list X -> list X -> Prop :=
  ...
  | disjoint_right : forall (x: X) (l1 l2: list X),
      ~ In x l1 -> disjoint l1 l2 -> disjoint l1 (x::l2).
...
(* FILL IN HERE *)
Theorem disjoint_nil_r: forall (X: Type) (l: list X),
    disjoint l [].
Proof.
  intros. induction l.
  - apply disjoint_nil.
...
Theorem disjoint_nil_l: forall (X: Type) (l: list X),
    disjoint [] l.
Proof.
  intros. induction l.
  - apply disjoint_nil.
...
Proof.
  ...
  - intros. simpl in H. destruct H as [H | H].
    ...
    + apply IHl' in H. destruct H as [l1 H]. destruct H as [l2 H].
      exists (n :: l1). exists l2. rewrite H. simpl. reflexivity.
Qed.
...
Inductive repeats {X:Type} : list X -> Prop :=
  (* FILL IN HERE *)
  | repeats_inf : forall (x: X) (l: list X),
...
Proof.
  ...
  - intros. inversion H0.
  - intros. apply repeats_inf.
    specialize IHl1' with l2. apply IHl1'.
...
(** **** Exercise: 3 stars, standard, optional (app_ne)
    ...
    use it later. *)
Lemma app_solv {X: Type} : forall (x: X) (l1 l2 l: list X),
  l1 ++ l2 = x :: l -> l1 = [] \/ exists l', l1 = x :: l'.
Proof.
  intros. induction l1.
  - left. reflexivity.
  - right. simpl in H. inversion H. exists l1. reflexivity.
Qed.
...
Proof.
  (* intros. split.
  - intros H. inversion H. simpl in H1.
    apply app_solv in H1. destruct H1 as [H1 | H1].
    + left. apply conj. 
      rewrite H1 in H3. apply H3.
      rewrite H1. simpl. apply H4.
    + right. destruct H1 as [s1'].
      exists s1'. exists s2. apply conj.
      * inversion H. 
        assert (G1: s0 = s1). *)
  Admitted.
(** [] *)
