...
Definition conj_fact : forall P Q R, P /\ Q -> Q /\ R -> P /\ R :=
  fun (P Q R: Prop) (HPQ: P /\ Q) (HQR: Q /\ R) =>
    conj (proj1 P Q HPQ) (proj2 Q R HQR).
(** [] *)
...
Definition ex_falso_quodlibet' : forall P, False -> P :=
  fun (P: Prop) contra => match contra with end.
(** [] *)
...
Definition eq_cons : forall (X : Type) (h1 h2 : X) (t1 t2 : list X),
  ...
  fun (X: Type) (h1 h2: X) (t1 t2: list X) (H: h1 == h2) (T: t1 == t2) =>
    match H with
...
Definition equality__leibniz_equality_term : forall (X : Type) (x y: X),
    x == y -> forall P : (X -> Prop), P x -> P y  :=
 fun (X: Type) (x y: X) (Hxy: x == y) (P: X -> Prop) =>
    match Hxy with
...
Definition double_neg : forall P : Prop,
    P -> ~~P 
  (* := fun (P: Prop) => fun HP => fun HNP => False. *)
  (* fun (P: Prop) (evH: P) => *)
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *). Admitted.  

Definition contradiction_implies_anything : forall P Q : Prop,
    (P /\ ~P) -> Q  
  (* := fun (P Q: Prop) (HPQ: P /\ ~P) => match HPQ with
                                    | conj HP HNP => 
                                    end. *)
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *). Admitted.  
...
Definition curry : forall P Q R : Prop,
    ((P /\ Q) -> R) -> (P -> (Q -> R))  :=
  fun (P Q R: Prop) (H: (P /\ Q) -> R) =>
    fun evP => fun evQ => H (conj evP evQ).
  

Definition uncurry : forall P Q R : Prop,
    (P -> (Q -> R)) -> ((P /\ Q) -> R)  :=
  fun (P Q R: Prop) (H: P -> (Q -> R)) =>
    fun evPQ => match evPQ with
                | conj evP evQ => H evP evQ
                end.
(** [] *)
...
Proof.
  intros PE. unfold proof_irrelevance.
  intros P pf1 pf2.
  assert (TP := PE P True).
  assert (H: P <-> True).
  { split. intros HP. apply I. intros I. apply pf1. }
  apply TP in H.
  generalize dependent pf2. generalize dependent pf1.
  rewrite H.
  intros pf1. destruct pf1.
  intros pf2. destruct pf2.
  reflexivity.
Qed.  
(** [] *)
