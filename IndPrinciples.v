...
(* FILL IN HERE
Theorem add_assoc'_ex : forall n m p : nat,
  n + (m + p) = (n + m) + p.
Proof.
...
Definition better_t_tree_ind_type : Prop
  :=  forall (X: Type) (P: t_tree X -> Prop),
      P t_leaf -> 
      (* (forall (t_l: t_tree X), P t_l ->
      forall (t_r: t_tree X), P t_r -> 
      forall (t_x: X), P (t_branch (t_l, t_x, t_r))) -> *)
      (forall (t_l: t_tree X) (t_x: X) (t_r: t_tree X), P t_l -> P t_r -> P (t_branch (t_l, t_x, t_r))) ->
...
Proof.
    intros X t.
    (* induction t as [|[[l v] r]]. *)
    induction t as [ | t_l t_x t_r P1 P2] using (better_t_tree_ind X).
    - reflexivity.
    - simpl. rewrite P1. rewrite P2. reflexivity.
Qed.
(** [] *)
